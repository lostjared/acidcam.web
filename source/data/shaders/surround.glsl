...
void main(void) {
    ...
    vec4 FragColorLeft = texture(textTexture, TexCoord + vec2(-1.0 / iResolution.x, 0.0));
    vec4 FragColorRight = texture(textTexture, TexCoord + vec2(1.0 / iResolution.x, 0.0));
    vec4 FragColorUp = texture(textTexture, TexCoord + vec2(0.0, 1.0 / iResolution.y));
    vec4 FragColorDown = texture(textTexture, TexCoord + vec2(0.0, -1.0 / iResolution.y));
    vec4 avgColor = (original_FragColor + FragColorLeft + FragColorRight + FragColorUp + FragColorDown) / 5.0;

    vec3 blended_FragColor = mix(avgColor.rgb, rainbow_FragColor, 0.5);
