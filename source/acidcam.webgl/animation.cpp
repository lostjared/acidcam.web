...
SDL_Surface* createSurfaceFromRGBA(unsigned char* data, int width, int height)
{
    // Determine the masks for the current CPU endianness
    ...
    // 32 bits per pixel, 4 bytes for RGBA
    int depth = 32;
    int pitch = width * 4;

    // Create the surface directly from the pixel buffer
    ...
    void processPixels(std::vector<uint8_t> &pixels, int w, int h) {

        try {
       
        SDL_Surface *surf = createSurfaceFromRGBA(pixels.data(), w, h);
        static GLuint texture = 0;
        main_w->setWindowSize(w, h);
        sprite.initSize(w, h);
        emscripten_set_canvas_element_size("#canvas", w,h);
        new_width = w;
        new_height = h;
        if(texture == 0) {
             texture = gl::createTexture(surf, true);
             sprite.initWithTexture(&shaders[shader_index], texture, 0, 0, w, h);
        }
        else {
            mx::Texture::flipSurface(surf);
            sprite.updateTexture(surf);
        }
...
EMSCRIPTEN_BINDINGS(image_loader) {
    ...
    emscripten::function("processPixels", &processPixels);
...
int main(int argc, char **argv) {
    ...
    MainWindow main_window("", 640, 480);
    main_w =&main_window;
